...
1. `工厂方法(Factory Method)模式`：
    定义一个创建对象的接口，但由子类决定实例化哪一个类。工厂方法使得子类实例化的过程推迟。
2. `抽象工厂(Abstract Factory)模式`：
    提供一个接口，用于创建一系列相关或相互依赖的对象，而无需指定它们的具体类。
3. `原型(Prototype)模式`：
    通过复制已有的原型实例来创建新对象，而不是直接通过类构造。
4. `单例(Singleton)模式`：
    保证一个类只有一个实例，并提供一个全局访问点来访问该实例。
5. `构建器(Builder)模式`：
    将一个复杂对象的构建与其表示分离，使得同样的构建过程能够创建不同的表示。

### 结构型模式

1. `适配器(Adapter)模式`：
    将一个类的接口转换成用户希望得到的另一个接口，使原本不兼容的接口能够协同工作。
2. `桥接(Bridge)模式`：
    将类的抽象部分和实现部分分离，使它们能够独立变化，避免类的继承树过于复杂。
3. `组合(Composite)模式`：
    将对象组合成树形结构以表示“整体-部分”的层次结构，使用户对单个对象和组合对象的使用一致。
4. `装饰(Decorator)模式`：
    动态地给一个对象添加额外的职责，而不改变其结构。提供比派生子类更灵活的扩展功能。
5. `外观(Facade)模式`：
    提供一个统一的接口，简化对子系统的使用，使得子系统变得更加易于使用。
6. `享元(Flyweight)模式`：
    通过共享细粒度的对象，减少内存消耗，适合大量相似对象的共享。
7. `代理(Proxy)模式`：
    为其他对象提供代理，以控制对象的访问，可以是延迟加载、权限控制等。

### 行为型模式

1. `职责链(Chain of Responsibility)模式`：
    将请求传递给多个对象处理，每个对象处理请求的机会，直到有一个对象处理它，减少请求者和接收者之间的耦合。
2. `命令(Command)模式`：
    将请求封装为一个对象，使得可以通过不同的请求对客户进行参数化，支持排队、记录日志和撤销操作。
3. `解释器(Interpreter)模式`：
    给定一种语言，定义它的语法表示，并定义一个解释器，用来解释该语言的句子。
4. `迭代器(Iterator)模式`：
    提供一种方法顺序访问聚合对象中的元素，而无需暴露该对象的内部表示。
5. `中介者(Mediator)模式`：
    用一个中介对象封装多个对象之间的交互，降低对象之间的耦合，使得它们可以独立地变更交互方式。
6. `备忘录(Memento)模式`：
    捕获一个对象的内部状态并将其保存，以便以后恢复到原来的状态，且不破坏封装性。
7. `观察者(Observer)模式`：
    定义对象间一对多的依赖关系，一个对象的状态变化时，所有依赖它的对象都会得到通知并自动更新。
8. `状态(State)模式`：
    允许一个对象根据其内部状态改变行为，不同的状态表现为类的不同实例。
9. `策略(Strategy)模式`：
    定义一系列算法，并将它们封装起来，使得算法可以相互替换，从而让客户端可以选择不同的算法。
10. `模板方法(Template Method)模式`：
     定义一个操作的算法骨架，将一些步骤延迟到子类中，使子类可以在不改变算法结构的情况下，重新定义算法的某些步骤。
11. `访问者(Visitor)模式`：
     表示一个作用于某对象结构中的各个元素的操作，使得在不改变元素类的前提下，能够定义对这些元素的新操作。
...
1. `单一职责原则`
    设计类时要尽量将职责范围控制在最小粒度内，使得类的功能明确、单一，不做多余的事情（实现高内聚，低耦合）。每个类应该只有一个原因去改变。
2. `开闭原则`
    软件实体（如类、模块、函数等）应该对扩展开放，对修改关闭。这意味着我们可以通过扩展已有的功能来添加新特性，而不必修改现有的代码。这有助于系统保持灵活性和可维护性。
3. `里氏替换原则`
    子类型必须能够替换掉它们的基类型。在程序中，如果基类和子类之间有继承关系，那么应该可以使用子类对象替换基类对象，而不会破坏程序的正确性和行为。
4. `接口隔离原则`
    不应该强迫一个类去实现它不需要的接口。一个类不应该对它用不到的方法负责。接口应该尽量小巧、单一，避免冗余的功能和无关的依赖。
5. `依赖倒置原则`
    高层模块不应该依赖于低层模块，而应该依赖于抽象。抽象不应该依赖于具体实现，具体实现应该依赖于抽象。例如，Java中定义了数据库操作的接口，各种数据库实现了这些接口，Java不直接依赖于具体数据库，而是依赖于这些抽象的接口。
6. `组合优于继承`
    继承往往会导致类之间的紧密耦合，而组合则能降低耦合度。继承关系是固定的，而通过组合，我们可以动态地传入不同的类对象，增加系统的灵活性，避免高耦合。
7. `组合复用原则`
   - 概念：优先使用组合（"contains a"）而非继承（"is a"）来实现复用。
   - 原因：继承会暴露实现细节，破坏封装性，导致“白箱复用”。而组合通过接口引用成员对象，实现“黑箱复用”，提高系统灵活性并减少耦合。
   - 优点：新类对象通过成员对象的接口访问其功能，使得系统更加灵活且耦合度更低；可以在运行时动态选择或替换成员对象。
   - 缺点：需要管理更多的对象，可能导致设计复杂度增加。
8. `迪米特法则(最少知识原则)`
   - 概念：一个对象应尽可能少地了解其他对象，避免与外部对象过多互动，减少对象之间的依赖和耦合。通常，只有直接的“朋友”对象（如`this`、方法参数、成员变量）才能与对象进行交互。
   - 要求：
     1. 优先考虑将类设计为不变类。
     2. 尽量降低类的访问权限，避免暴露内部实现。
     3. 谨慎使用序列化（Serializable），避免无意识地将内部实现暴露给外部。
     4. 降低成员变量的访问权限，避免不必要的公开接口。
   - 优点：减少类之间的耦合，提升系统的灵活性和可维护性。
   - 缺点：过多使用迪米特法则可能导致设计复杂化，生成大量中介类。